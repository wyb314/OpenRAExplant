...
namespace Engine.Physics
{
    ...
    using Settings = TrueSync.Physics2D.Settings;
    public class Physics2DWorldManager : IPhysicsManager
    {
        ...
        Dictionary<IBody, IRegidbodyWrapObject> gameObjectMap;
        ...
        public void Init()
        {
            ...
            gameObjectMap = new Dictionary<IBody, IRegidbodyWrapObject>();
            collisionInfo = new Dictionary<Body, Dictionary<Body, TSCollision2D>>();
        ...
        public void AddBody(ICollider iCollider)
        {
            ...
            {
                //Log.LogError("You have a 3D object but your Physics 3D is disabled.");
                return;
            }
            ...
            gameObjectMap[tsCollider._body] = tsCollider.agent.RendererObject;
        ...
        private void CollisionDetected(Body body1, Body body2, TrueSync.Physics2D.Contact contact, string callbackName)
        {
            ...
            IRegidbodyWrapObject b1 = gameObjectMap[body1];
            IRegidbodyWrapObject b2 = gameObjectMap[body2];
            ...
            b1.SendCollisionMessage(callbackName, GetCollisionInfo(body1, body2, contact));
            b2.SendCollisionMessage(callbackName, GetCollisionInfo(body2, body1, contact));

            //TrueSyncManager.UpdateCoroutines();
        }

        private TSCollision2D GetCollisionInfo(Body body1, Body body2, TrueSync.Physics2D.Contact c)
        {
            if (!collisionInfo.ContainsKey(body1))
            {
                collisionInfo.Add(body1, new Dictionary<Body, TSCollision2D>());
            }

            Dictionary<Body, TSCollision2D> collisionInfoBody1 = collisionInfo[body1];
        ...
        public IRegidbodyWrapObject GetGameObject(IBody body)
        {
        ...
        {
            IRegidbodyWrapObject go = GetGameObject(body);
            if (go == null)
            {
                return -1;
            }
        ...
        public TSRaycastHit2D Raycast(TSVector2 origin, TSVector2 direction, FP distance)
        {
            ...
            Func<TrueSync.Physics2D.Fixture, TSVector2, TSVector2, FP, FP> callback = delegate (TrueSync.Physics2D.Fixture fixture, TSVector2 point, TSVector2 normal, FP fraction) {
                result = new TSRaycastHit2D(gameObjectMap[fixture.Body].GetComponentByType(typeof(TSCollider2D)) as TSCollider2D);
                return 0;
            };
        ...
        public TSRaycastHit2D[] RaycastAll(TSVector2 origin, TSVector2 direction, FP distance)
        {
            ...
            Func<TrueSync.Physics2D.Fixture, TSVector2, TSVector2, FP, FP> callback = delegate (TrueSync.Physics2D.Fixture fixture, TSVector2 point, TSVector2 normal, FP fraction) {
                result.Add(new TSRaycastHit2D(gameObjectMap[fixture.Body].GetComponentByType(typeof(TSCollider2D)) as TSCollider2D));
                return -1;
            };
