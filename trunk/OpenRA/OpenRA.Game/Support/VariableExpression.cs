...
namespace OpenRA.Support
{
    public abstract class VariableExpression
    {
        ...
        static IEnumerable<TokenTypeInfo> CreateTokenTypeInfoEnumeration()
        {
            for (var i = 0; i <= (int)TokenType.Invalid; i++)
            {
                ...
                throw new InvalidProgramException("CreateTokenTypeInfoEnumeration is missing a TokenTypeInfo entry for TokenType.{0}".F(
                    Exts.Enum<TokenType>.GetValues()[i]));
            }
        }
        ...
        class Token
        {
                    ...
                    for (; i < expression.Length; i++)
                    {
                        ...
                        if (cc != CharClass.Digit)
                        {
                            if (cc != CharClass.Whitespace && cc != CharClass.Operator && cc != CharClass.Mixed)
                                throw new Exception("Number {0} and variable merged at index {1}".F(
            ...
            static TokenType VariableOrKeyword(string expression, int start, ref int i)
            {
                if (CharClassOf(expression[i - 1]) == CharClass.Mixed)
                    throw new Exception("Invalid identifier end character at index {0} for `{1}`".F(
            ...
            public static TokenType GetNextType(string expression, ref int i, TokenType lastType = TokenType.Invalid)
            {
                ...
                switch (expression[i])
                {
                    ...
                    case '=':
                        ...
                        throw new Exception("Unexpected character '=' at index {0} - should it be `==`?".F(start));
                ...
                if (cc != CharClass.Id)
                    throw new Exception("Invalid character '{0}' at index {1}".F(expression[i], start));
            ...
            public static Token GetNext(string expression, ref int i, TokenType lastType = TokenType.Invalid)
            {
                ...
                if (CharClassOf(expression[i]) == CharClass.Whitespace)
                {
                    ...
                    throw new Exception("Missing whitespace at index {0}, after `{1}` operator."
                ...
                if (!whitespaceBefore && RequiresWhitespaceBefore(type))
                    throw new Exception("Missing whitespace at index {0}, before `{1}` operator."
        ...
        Expression Build(ExpressionType resultType)
        {
            ...
            for (var i = 0; ;)
            {
                ...
                if (token == null)
                {
                    ...
                    if (lastToken.RightOperand)
                        throw new Exception("Missing value or sub-expression at end for `{0}` operator".F(lastToken.Symbol));
                ...
                if (token.Closes != Grouping.None)
                {
                    if (currentOpeners.Count == 0)
                        throw new Exception("Unmatched closing parenthesis at index {0}".F(token.Index));
                ...
                if (lastToken == null)
                {
                    ...
                    if (token.LeftOperand)
                        throw new Exception("Missing value or sub-expression at beginning for `{0}` operator".F(token.Symbol));
                }
                else
                {
                    // Disallow empty parentheses
                    if (lastToken.Opens != Grouping.None && token.Closes != Grouping.None)
                        throw new Exception("Empty parenthesis at index {0}".F(lastToken.Index));
                    ...
                    if (lastToken.RightOperand == token.LeftOperand)
                    {
                        ...
                        throw new Exception("Missing binary operation before `{0}` at index {1}".F(token.Symbol, token.Index));
                    }
                }
            ...
            if (currentOpeners.Count > 0)
                throw new Exception("Unclosed opening parenthesis at index {0}".F(currentOpeners.Peek().Index));
        ...
        protected Func<IReadOnlyDictionary<string, int>, T> Compile<T>()
        {
            ...
            return System.Linq.Expressions.Expression.Lambda<Func<IReadOnlyDictionary<string, int>, T>>(Build(resultType), SymbolsParam).Compile();
        }
        ...
        static readonly ParameterExpression SymbolsParam =
            System.Linq.Expressions.Expression.Parameter(typeof(IReadOnlyDictionary<string, int>), "symbols");
        static readonly ConstantExpression Zero = System.Linq.Expressions.Expression.Constant(0);
        static readonly ConstantExpression One = System.Linq.Expressions.Expression.Constant(1);
        static readonly ConstantExpression False = System.Linq.Expressions.Expression.Constant(false);
        static readonly ConstantExpression True = System.Linq.Expressions.Expression.Constant(true);

        static Expression AsBool(Expression expression)
        {
            return System.Linq.Expressions.Expression.NotEqual(expression, Zero);
        }
        ...
        class AstStack
        {
            ...
            public Expression Peek(ExpressionType toType)
            {
                ...
                throw new InvalidProgramException("Unable to convert ExpressionType.{0} to ExpressionType.{1}".F(
                    Exts.Enum<ExpressionType>.GetValues()[(int)fromType], Exts.Enum<ExpressionType>.GetValues()[(int)toType]));
            }
            ...
            public Expression Build(Token[] postfix, ExpressionType resultType)
            {
                foreach (var t in postfix)
                {
                    switch (t.Type)
                    {
                        case TokenType.And:
                            {
                                ...
                                ast.Push(System.Linq.Expressions.Expression.And(x, y));
                                continue;
                            }
                        ...
                        case TokenType.Or:
                            {
                                var y = ast.Pop(ExpressionType.Bool);
                                var x = ast.Pop(ExpressionType.Bool);
                                ast.Push(System.Linq.Expressions.Expression.Or(x, y));
                                continue;
                            }
                        ...
                        case TokenType.NotEquals:
                            {
                                ...
                                ast.Push(System.Linq.Expressions.Expression.NotEqual(x, y));
                                continue;
                            }
                        ...
                        case TokenType.Equals:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.Equal(x, y));
                                continue;
                            }

                        case TokenType.Not:
                            {
                                ast.Push(System.Linq.Expressions.Expression.Not(ast.Pop(ExpressionType.Bool)));
                                continue;
                            }

                        case TokenType.Negate:
                            {
                                ast.Push(System.Linq.Expressions.Expression.Negate(ast.Pop(ExpressionType.Int)));
                                continue;
                            }

                        //case TokenType.OnesComplement:
                        //    {
                        //        ast.Push(System.Linq.Expressions.Expression.OnesComplement(ast.Pop(ExpressionType.Int)));
                        //        continue;
                        //    }
                        ...
                        case TokenType.LessThan:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.LessThan(x, y));
                                continue;
                            }
                        ...
                        case TokenType.LessThanOrEqual:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.LessThanOrEqual(x, y));
                                continue;
                            }
                        ...
                        case TokenType.GreaterThan:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.GreaterThan(x, y));
                                continue;
                            }
                        ...
                        case TokenType.GreaterThanOrEqual:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.GreaterThanOrEqual(x, y));
                                continue;
                            }
                        ...
                        case TokenType.Add:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.Add(x, y));
                                continue;
                            }
                        ...
                        case TokenType.Subtract:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.Subtract(x, y));
                                continue;
                            }
                        ...
                        case TokenType.Multiply:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                ast.Push(System.Linq.Expressions.Expression.Multiply(x, y));
                                continue;
                            }
                        ...
                        case TokenType.Divide:
                            {
                                var y = ast.Pop(ExpressionType.Int);
                                var x = ast.Pop(ExpressionType.Int);
                                var isNotZero = System.Linq.Expressions.Expression.NotEqual(y, Zero);
                                var divide = System.Linq.Expressions.Expression.Divide(x, y);
                        ...
                        case TokenType.Modulo:
                            {
                                ...
                                var x = ast.Pop(ExpressionType.Int);
                                var isNotZero = System.Linq.Expressions.Expression.NotEqual(y, Zero);
                                var modulo = System.Linq.Expressions.Expression.Modulo(x, y);
                        ...
                        case TokenType.Number:
                            {
                                ast.Push(System.Linq.Expressions.Expression.Constant(((NumberToken)t).Value));
                                continue;
                            }

                        case TokenType.Variable:
                            {
                                var symbol = System.Linq.Expressions.Expression.Constant(((VariableToken)t).Symbol);
                                Func<string, IReadOnlyDictionary<string, int>, int> parseSymbol = ParseSymbol;
                                ast.Push(System.Linq.Expressions.Expression.Call(parseSymbol.Method, symbol, SymbolsParam));
                                continue;
                            }
                        ...
                        default:
                            throw new InvalidProgramException(
                                "ConditionExpression.Compiler.Compile() is missing an expression builder for TokenType.{0}".F(
                                    Exts.Enum<TokenType>.GetValues()[(int)t.Type]));
                    }
                }
