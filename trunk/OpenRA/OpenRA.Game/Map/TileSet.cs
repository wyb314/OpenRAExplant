            ...
            if (!PickAny)
            {
                ...
                foreach (var node in nodes)
                {
                    ...
                    if (!int.TryParse(node.Key, out key) || key < 0 || key >= tileInfo.Length)
                        throw new Exception("Invalid tile key '{0}' on template '{1}' of tileset '{2}'.".F(node.Key, Id, tileSet.Id));
    ...
    public class TileSet
    {
        ...
        public TileSet(IReadOnlyFileSystem fileSystem, string filepath)
        {
            ...
            if (TerrainInfo.Length >= byte.MaxValue)
                throw new Exception("Too many terrain types.");
            ...
            for (byte i = 0; i < TerrainInfo.Length; i++)
            {
                ...
                if (terrainIndexByType.ContainsKey(tt))
                    throw new Exception("Duplicate terrain type '{0}' in '{1}'.".F(tt, filepath));
        ...
        public TileSet(string name, string id, string palette, TerrainTypeInfo[] terrainInfo)
        {
            ...
            for (byte i = 0; i < terrainInfo.Length; i++)
            {
                var tt = terrainInfo[i].Type;
                if (terrainIndexByType.ContainsKey(tt))
                    throw new Exception("Duplicate terrain type '{0}'.".F(tt));
        ...
        public byte GetTerrainIndex(string type)
        {
            ...
            throw new Exception("Tileset '{0}' lacks terrain type '{1}'".F(Id, type));
        }
