...
namespace ConsoleTestApp
{
	class Program
	{
		...
		{
			DNA.DefaultMutationProbability = 0.15;
			DNA test1 = new DNA(5);
			DNA test2 = new DNA(5);
			...
			DNA<Person> testPerson1 = new DNA<Person>();
			DNA<Person> testPerson2 = new DNA<Person>();

			Console.Clear();
			Console.WriteLine("Test DNA Person 1:");
			PrintPerson(testPerson1);
			Console.WriteLine();
			Console.WriteLine("Test DNA Person 2:");
			PrintPerson(testPerson2);

			Console.WriteLine(Environment.NewLine);
			Console.WriteLine("Child 1 of DNA Person 1 & DNA Person 2:");
			PrintPerson(new DNA<Person>(testPerson1, testPerson2));
			Console.WriteLine();
			Console.WriteLine("Child 2 of DNA Person 1 & DNA Person 2:");
			PrintPerson(new DNA<Person>(testPerson1, testPerson2));
			...
			Func<Vector4<double>, double> fitness = (dna) =>
				{
					Vector4<double> v = dna;
					double res = v.X * v.X + (2 * v.Y) / v.Z + v.W;
					return -Math.Abs(res - 1);
				};
			Func<Vector4<double>, bool> isGoalMet = (dna) =>
				{
					return Math.Round(fitness(dna), 3) == 0;
				};
			int maxGenerations = 1000;
			int populationSize = 100;
			var generations = GeneticAlgorithm.Run(populationSize, maxGenerations, 2, 0.1, fitness, isGoalMet);
		...
		private static void PrintFormula(Vector4<double> dna)
		{
			Console.WriteLine("x = {0:0.000}, y = {1:0.000}, z = {2:0.000}, w = {3:0.000}", dna.X, dna.Y, dna.Z, dna.W);
			Console.WriteLine("{0:0.000}^2 + (2 * {1:0.000}) / {2:0.000} + {3:0.000} = {4:0.000}" + Environment.NewLine,
				dna.X, dna.Y, dna.Z, dna.W, dna.X * dna.X + (2 * dna.Y) / dna.Z + dna.W);
		}

		private static void PrintTestDNA(DNA dna)
		{
			Console.Write("Hair: ");
			if (dna[1] < byte.MaxValue / 3)
				Console.WriteLine("Brown");
			else if(dna[1] < byte.MaxValue / 3 * 2)
				Console.WriteLine("Blonde");
			else
				Console.WriteLine("Red");

			Console.Write("Eyes: ");
			if(dna[2] < byte.MaxValue / 3)
				Console.WriteLine("Brown");
			else if (dna[2] < byte.MaxValue / 3 * 2)
				Console.WriteLine("Green");
			else
				Console.WriteLine("Blue");

			Console.Write("Gender: ");
			if(dna[3] < byte.MaxValue / 2)
				Console.WriteLine("Male");
			else
				Console.WriteLine("Female");

			Console.Write("Height: ");
			double feet = ((double)dna[4] * 2) / byte.MaxValue;
			feet += 4.5;
			Console.WriteLine("{0:0.0} feet", feet);

			Console.Write("Weight: ");
			double lbs = ((double)dna[4] * 200) / byte.MaxValue;
			lbs += 100;
		...
		{
			Console.WriteLine("Hair: {0}", person.HairColor);
			Console.WriteLine("Eyes: {0}", person.EyeColor);
			Console.WriteLine("Gender: {0}", person.Gender);
			Console.WriteLine("Height: {0:0.0} feet", person.Height);
			Console.WriteLine("Weight: {0:0.0} lbs", person.Weight);
		}
		...
		private static PlannerActionSet InitializeGOAP()
		{
			...
			Extensions<Food>.IsLeftEqualToRight = (f1, f2) => { return (f1 & f2) == f2; };
		...
		private static void TestHeapsort(List<int> ints)
		{
			...
			foreach (int num in DataAlgorithms.Heapsort(ints, (x, y) =>
			{
				if (x > y)
					return -1;
				if (x == y)
