...
namespace OpenRA
{
    ...
    public class ExternalMods : IReadOnlyDictionary<string, ExternalMod>
    {
        ...
        internal void Register(Manifest mod, string launchPath, ModRegistration registration)
        {
            ...
            if (HasFlag(registration,ModRegistration.System))
                sources.Add(Platform.SystemSupportDir);

            if (HasFlag(registration,ModRegistration.User))
        ...
        private bool HasFlag(ModRegistration sourceFlag , ModRegistration destinationFlag)
        {
        ...
        internal void Unregister(Manifest mod, ModRegistration registration)
        {
            ...
            foreach (var source in sources.Distinct())
            {
                var path = Path.Combine(source, "ModMetadata");
                path = Path.Combine(path, key + ".yaml");
                try
                {
                    if (File.Exists(path))
                        File.Delete(path);
